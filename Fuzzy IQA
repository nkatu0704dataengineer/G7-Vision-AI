...
class FuzzyFaceQualityAssessor:
    def __init__(self):
        ...
        # =============================
        # 1. DEFINE UNIVERSE RANGE
        # =============================

        self.blur = ctrl.Antecedent(np.arange(0, 1001, 1), 'blur')        # Variance of Laplacian
        self.brightness = ctrl.Antecedent(np.arange(0, 256, 1), 'brightness')  # Mean intensity
        self.contrast = ctrl.Antecedent(np.arange(0, 101, 1), 'contrast') # RMS / GCF contrast
        self.noise = ctrl.Antecedent(np.arange(0, 51, 1), 'noise')        # Estimated noise level

        self.quality = ctrl.Consequent(np.arange(0, 101, 1), 'quality')   # Final IQA score
        ...
        self.blur['medium'] = fuzz.trimf(self.blur.universe, [200, 450, 700])
        ...
        rules = [
            ...
            ctrl.Rule(self.blur['medium'] & self.noise['medium'], self.quality['poor']),
